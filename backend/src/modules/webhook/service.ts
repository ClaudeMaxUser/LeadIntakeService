...
export class WebhookService {
  async processLeadWebhook(
    ...
    return await withTransaction(async (client) => {
      // Check if external_lead_id already exists (idempotency check)
      const existingQuery = `
        SELECT id FROM leads WHERE external_lead_id = $1
      `;
      const existingResult = await client.query<{ id: string }>(existingQuery, [extracted.leadgen_id]);

      if (existingResult.rows.length > 0) {
        const existingId = existingResult.rows[0].id;
        // Optionally log duplicate ignore activity or record idempotency audit
